...
def minimal_preprocessing(data):
    # removing metadata
...
def lowercase(data):
    data["article"] = data["article"].lower()
...
def remove_stopwords(data):
    ...
    data["article"] = " ".join([word for word in data["article"].split() if word not in stop_words])
    #data["highlights"] = " ".join([word for word in data["highlights"].split() if word not in stop_words])
    return data

def remove_punctuation(data):
    data["article"] = data["article"].translate(str.maketrans("", "", string.punctuation))
